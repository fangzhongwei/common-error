package com.jxjxgo.common.exception;

/**
 * Created by fangzhongwei on 2016/11/29.
 */
public enum ErrorCode {

    EC_SYSTEM_ERROR("EC_SYSTEM_ERROR", "系统目前无法处理您的请求，请稍后再试。"),
    EC_INVALID_REQUEST("EC_INVALID_REQUEST", "不正确的请求，请检查输入。"),
    EC_ACCESS_DENY("EC_ACCESS_DENY", "拒绝访问。"),

    EC_UC_INVALID_MOBILE("EC_UC_INVALID_MOBILE", "手机号格式不正确，请修改或重新输入。"),
    EC_UC_INVALID_EMAIL("EC_UC_INVALID_EMAIL", "邮箱格式不正确，请修改或重新输入。"),
    EC_UC_USERNAME_TAKEN("EC_UC_USERNAME_TAKEN", "用户名已被占用。"),
    EC_UC_MOBILE_TAKEN("EC_UC_MOBILE_TAKEN", "手机号已被占用。"),
    EC_UC_EMAIL_TAKEN("EC_UC_EMAIL_TAKEN", "邮箱已被占用。"),
    EC_UC_INVALID_UN_OR_PWD("EC_UC_INVALID_UN_OR_PWD", "用户名或密码不正确。"),
    EC_UC_INVALID_PWD("EC_UC_INVALID_PWD", "密码不正确。"),
    EC_UC_INVALID_CHECK_CODE("EC_UC_INVALID_CHECK_CODE", "校验码不正确。"),
    EC_UC_MEMBER_LOCKED("EC_UC_MEMBER_LOCKED", "您的账户已锁定。"),

    EC_UC_PROFILE_UN_MATCHED("EC_UC_PROFILE_UN_MATCHED", "用户资料验证未通过。"),

    EC_UC_MEMBER_ACCOUNT_FREEZE("EC_UC_MEMBER_ACCOUNT_FREEZE", "您的账户已冻结。"),
    EC_UC_MEMBER_NOT_EXISTS("EC_UC_MEMBER_NOT_EXISTS", "会员信息未找到。"),

    EC_UC_INVALID_USERNAME("EC_UC_INVALID_USERNAME", "用户名不合法。"),

    EC_SSO_SESSION_EXPIRED("EC_SSO_SESSION_EXPIRED", "会话已过期，请重新登录。"),
    EC_SSO_SESSION_REPELLED("EC_SSO_SESSION_REPELLED", "您的账号已在其他设备登录。"),
    EC_SSO_TOKEN_DEVICE_MISMATCH("EC_SSO_TOKEN_DEVICE_MISMATCH", "会话与当前设备不相符。"),

    EC_ED_TICKET_NOT_EXISTS("EC_ED_TICKET_NOT_EXISTS", "ticket不存在。"),

    EC_GC_CONFIG_ERROR("EC_GC_CONFIG_ERROR", "系统配置错误。"),
    EC_SMS_TYPE_CONFIG_ERROR("EC_SMS_TYPE_CONFIG_ERROR", "信息类型配置错误。"),
    EC_SMS_DAY_LIMIT_ERROR("EC_SMS_DAY_LIMIT_ERROR", "手机号当天获取验证码次数超过系统限制。"),
    EC_SMS_SEND_TOO_OFTEN_ERROR("EC_SMS_SEND_TOO_OFTEN_ERROR", "验证码发送过于频繁。"),

    EC_SMS_WRONG_CODE("EC_SMS_WRONG_CODE", "验证码不正确。"),
    EC_SMS_CODE_EXPIRED_ERROR("EC_SMS_CODE_EXPIRED_ERROR", "验证码已过期。"),

    EC_GAME_INVALID_RESULT("EC_GAME_INVALID_RESULT", "游戏结算结果不正确。"),
    EC_ORDER_PAYMENT_ORDER_NOT_FOUND("EC_ORDER_PAYMENT_ORDER_NOT_FOUND", "支付订单不存在。"),
    EC_ORDER_ORDER_FINISHED("EC_ORDER_ORDER_FINISHED", "订单已结束。"),
    EC_ORDER_AMOUNT_NOT_MATCHED("EC_ORDER_AMOUNT_NOT_MATCHED", "支付金额与订单金额不一致。"),

    EC_ACCOUNT_NOT_EXIST("EC_ACCOUNT_NOT_EXIST", "账户不存在。"),

    EC_I18N_LAN_NOT_EXIST("EC_I18N_LAN_NOT_EXIST", "语言不存在。"),

    EC_GAME_NOT_ENOUGH_DIAMOND("EC_GAME_NOT_ENOUGH_DIAMOND", "您的钻石不足，无法进入该房间。"),

    EC_GAME_DIAMOND_TOO_MUCH("EC_GAME_DIAMOND_TOO_MUCH", "您的钻石太多，无法进入该房间。"),

    EC_GAME_INVALID_REQUEST_DATA("EC_GAME_INVALID_REQUEST_DATA", "请求数据不合法。"),
    EC_GAME_GAME_NOT_EXIST("EC_GAME_GAME_NOT_EXIST", "游戏不存在。"),
    PLAY_GAME_CARDS_NOT_BIG_ENOUGH("PLAY_GAME_CARDS_NOT_BIG_ENOUGH", "您的牌太小了。"),


    MSG_MSG_NOT_EXISTS("MSG_MSG_NOT_EXISTS", "没有找到信息。"),
    MSG_MSG_ALREADY_REPLAYED("MSG_MSG_ALREADY_REPLAYED", "信息已经回复。"),

    MSG_VC_VOUCHER_NOT_EXISTS("MSG_VC_VOUCHER_NOT_EXISTS", "没有找到凭证。"),

    EC_CAP_CAPTCHA_EXPIRED("EC_CAP_CAPTCHA_EXPIRED", "验证码已过期。"),
    EC_CAP_INVALID_CAPTCHA("EC_CAP_INVALID_CAPTCHA", "验证码不正确。"),

    EC_BUS_BANK_CARD_NOT_FOUND("EC_BUS_BANK_CARD_NOT_FOUND", "银行卡未找到。"),
    EC_BUS_ACCOUNT_NOT_FOUND("EC_BUS_ACCOUNT_NOT_FOUND", "账户未找到。"),

    EC_BUS_ACCOUNT_FREEZE_IN("EC_BUS_ACCOUNT_FREEZE_IN", "抱歉，您的账户已冻结充值功能。"),
    EC_BUS_ACCOUNT_FREEZE_OUT("EC_BUS_ACCOUNT_FREEZE_OUT", "抱歉，您的账户已冻结提现功能。"),

    EC_BUS_DEPOSIT_ORDER_NOT_FOUND("EC_BUS_DEPOSIT_ORDER_NOT_FOUND", "充值订单未找到。"),
    EC_BUS_WITHDRAW_ORDER_NOT_FOUND("EC_BUS_WITHDRAW_ORDER_NOT_FOUND", "提现订单未找到。"),
    EC_BUS_ORDER_HANDED("EC_BUS_ORDER_HANDED", "订单已处理。"),
    EC_BUS_ORDER_STATUS_NOT_WAITING_HANDLE("EC_BUS_ORDER_STATUS_NOT_WAITING_HANDLE", "订单状态不是待处理。"),
    EC_BUS_ORDER_STATUS_NOT_WAITING_DEPOSIT("EC_BUS_ORDER_STATUS_NOT_WAITING_DEPOSIT", "充值时间已经提交。"),
    EC_BUS_DEPOSIT_BANK_WATER_USED("EC_BUS_DEPOSIT_BANK_WATER_USED", "银行流水号已经入款！"),
    EC_BUS_WITHDRAW_BANK_WATER_USED("EC_BUS_WITHDRAW_BANK_WATER_USED", "银行流水号已经出款！"),

    EC_BUS_NOT_ENOUGH_MAY_WITHDRAW_AMOUNT("EC_BUS_NOT_ENOUGH_MAY_WITHDRAW_AMOUNT", "可提现余额不足！"),
    EC_BUS_WITHDRAW_ORDER_STATUS_NOT_WAITING_HANDLE("EC_BUS_WITHDRAW_ORDER_STATUS_NOT_WAITING_HANDLE", "提现订单状态不是待处理状态。"),

    DB_UPDATE_FAILURE("DB_UPDATE_FAILURE", "数据更新失败。"),
    DB_INSERT_FAILURE("DB_INSERT_FAILURE", "数据插入失败。"),

    DB_CACHE_EXPIRE_TIME_MUST_BIGGER_THAN_ZERO("DB_CACHE_EXPIRE_TIME_MUST_BIGGER_THAN_ZERO", "缓存过期时间必须大于0。"),
    DB_CACHE_KEY_TOO_LONG("DB_CACHE_KEY_TOO_LONG", "缓存Key太长。"),
    DB_CACHE_VALUE_TOO_LONG("DB_CACHE_VALUE_TOO_LONG", "缓存Value太长。"),

    EC_BET_BET_ITEM_NOT_EXIST("EC_BET_BET_ITEM_NOT_EXIST", "投注项未找到。"),
    EC_BET_BET_ITEM_CHANGED("EC_BET_BET_ITEM_CHANGED", "投注项已发生改变。"),
    EC_BET_BET_ITEM_ODDS_CHANGED("EC_BET_BET_ITEM_ODDS_CHANGED", "赔率已发生改变。"),

    EC_BET_NOT_ENOUGH_BALANCE("EC_BET_NOT_ENOUGH_BALANCE", "您的余额不足，请先存款。"),
    EC_BET_INVALID_BET("EC_BET_INVALID_BET", "投注无效。"),
    EC_BET_ORDER_NOT_EXIST("EC_BET_ORDER_NOT_EXIST", "投注订单不存在。"),
    EC_BET_SETTLE_ERROR("EC_BET_SETTLE_ERROR", "结算错误。"),

    EC_BET_GAME_NOT_EXIST("EC_BET_GAME_NOT_EXIST", "赛事不存在。"),
    EC_BET_GAME_HALF_NOT_FINISHED_YET("EC_BET_GAME_HALF_NOT_FINISHED_YET", "赛事半场还未结束。"),
    EC_BET_GAME_NOT_FINISHED_YET("EC_BET_GAME_NOT_FINISHED_YET", "赛事还未结束。"),

    EC_BET_ORDER_ALREADY_SETTLED("EC_BET_ORDER_ALREADY_SETTLED", "投注单已结算。"),

    EC_SPORT_NO_DATA("EC_SPORT_NO_DATA", "没有数据。");

    private String code;
    private String desc;

    ErrorCode(String code, String desc) {
        this.code = code;
        this.desc = desc;
    }

    public String getCode() {
        return code;
    }

    public String getDesc() {
        return desc;
    }

    public static ErrorCode get(String code) {
        final ErrorCode[] values = ErrorCode.values();
        for (int i = 0; i < values.length; i++) {
            if (values[i].getCode().equals(code)) return values[i];
        }
        return null;
    }
}
